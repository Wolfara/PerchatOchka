...
public class Player2LVL : MonoBehaviour
{
    ...
    float timer, bPUB90 = 300, minJump, minMizinec, minBPalec;
    int mizSdv = 334, lock360 = 360, bPSdv = 26;
    ...
    void Update()
    {
        ...
        if (minJump < 200)
            up.text = "180";
        cam.transform.position = new Vector3(transform.position.x - 4, transform.position.y + 2.7f, 101.5f);
        if (gameStarted)
        {
